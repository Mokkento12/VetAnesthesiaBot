    ...
    for update := range updates {
        if update.Message != nil {
            ...
            if update.Message.Text == "/start" {
                // Создаём Reply-клавиатуру с кнопками "Рассчитать дозу", "Перезапустить расчёт", "Очистить чат" и "Помощь"
            ...
            case "Перезапустить расчёт":
                ...
                bot.Send(msg)
            case "Очистить чат":
                // Отправляем сообщение, информирующее о необходимости очистки чата
                msg := tgbotapi.NewMessage(update.Message.Chat.ID, "Чтобы очистить чат, удалите старые сообщения вручную.")
                bot.Send(msg)
                // Сбрасываем шаги
                step = 0
                msg = tgbotapi.NewMessage(update.Message.Chat.ID, "Нажмите 'Рассчитать дозу' для нового расчёта.")
                bot.Send(msg)
            case "Помощь":
                // Отправляем сообщение с инструкциями
                msg := tgbotapi.NewMessage(update.Message.Chat.ID, "Этот бот поможет вам рассчитать дозу анестезии для животных. Следуйте инструкциям:\n\n1. Нажмите 'Рассчитать дозу'.\n2. Введите вес животного, дозу на кг и концентрацию препарата.\n3. Бот вычислит нужное количество препарата и выведет результат.\n\nВы также можете использовать кнопку 'Перезапустить расчёт' для начала нового расчёта или 'Очистить чат' для очистки старых сообщений.")
                bot.Send(msg)
                // Сбрасываем шаги
                step = 0
